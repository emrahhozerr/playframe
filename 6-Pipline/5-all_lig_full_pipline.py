...
COK LIGLI DESTEK:
players.csv / teams.csv / tags2name.csv / player_games.csv / eventid2name.csv
TUM ligler icin ORTAK (global) dosyalar - tek sefer okunur.
matches_<Lig>.csv / events_<Lig>.csv ise HER LIG ICIN AYRI dosyalar - LEAGUES
listesindeki her isim icin raw_data/ klasorunde matches_<isim>.csv ve
events_<isim>.csv olarak aranir (ornek: matches_Spain.csv, events_Spain.csv).

KULLANIM:
1) Bu dosyayla ayni klasore "raw_data" adinda bir klasor ac, ham Wyscout
   dosyalarini oraya koy:
   - Ortak/global: players.csv, teams.csv, tags2name.csv, player_games.csv,
     eventid2name.csv
   - Lig basina: matches_<Lig>.csv, events_<Lig>.csv (LEAGUES listesindeki
     her isim icin, ornek: matches_England.csv, events_England.csv,
     matches_Spain.csv, events_Spain.csv, ...)
2) `python full_pipeline.py` calistir.
3) Butun ara/nihai ciktilar bu dosyayla ayni klasordeki "data" klasorune yazilir,
   ve her satirda hangi ligden geldigini gosteren bir "league_code" kolonu olur.
"""
...
# islenecek ligler - yeni bir lig eklemek icin sadece buraya isim eklemen
# ve raw_data/ altina matches_<isim>.csv + events_<isim>.csv koyman yeterli
LEAGUES = ["England", "Spain", "Italy", "Germany", "France"]

# ortak/global (tum liglerde ayni) ham dosyalar
...
def league_matches_csv(league_code):
    """Lig basina degisen ham mac dosyasinin yolu, ornek: raw_data/matches_Spain.csv"""
    return os.path.join(RAW_DATA_DIR, f"matches_{league_code}.csv")


def league_events_csv(league_code):
    """Lig basina degisen ham event dosyasinin yolu, ornek: raw_data/events_Spain.csv"""
    return os.path.join(RAW_DATA_DIR, f"events_{league_code}.csv")
...
def matches_eng_pipline(matches_eng, eutc, league_code=None, csv=True):
    """
    eut değeri ilgili data sette ne is o girilcek.
    league_code verilirse (ör. "Spain"), cok-ligli calisma icin sonuc
    tabloya bir "league_code" kolonu eklenir - boylece bu ligin maclari
    diger liglerle birlestirilince ayirt edilebilir.
    csv=False verilirse diske yazmaz, sadece DataFrame dondurur (cok-ligli
    dongude her ligi ayri ayri diske yazip birbirinin ustune yazmamak icin).
    """
    import pandas as pd
    import numpy as np
    import ast
    ...
    if league_code is not None:
        df_new["league_code"] = league_code
...
def events_eng_pipline(events_eng, league_code=None, csv=True):
    """
    league_code verilirse (ör. "Spain"), sonuc tabloya bir "league_code"
    kolonu eklenir. csv=False verilirse diske yazmaz, sadece DataFrame
    dondurur (cok-ligli dongude kullanilir, bkz. matches_eng_pipline).

    NOT (cok-ligli birlestirme): tag_XXX dummy kolonlari SADECE bu ligin
    verisinde GECEN tag'lerden uretiliyor (pd.crosstab), yani her ligin
    tag kolon seti farkli olabilir. Farkli ligleri pd.concat ile
    birlestirdikten sonra eksik tag_ kolonlari NaN kalir - main() bunlari
    concat sonrasi topluca fillna(0) ile duzeltiyor, tek basina bu
    fonksiyon icinde degil.
    """
    import pandas as pd
    import numpy as np
    import ast
    ...
    if league_code is not None:
        dataframe["league_code"] = league_code
...
def perc90_builder(event_eng, player_game, csv=False):
    ...
    # cok-ligli calisildiysa (matches_eng_pipline'a league_code verildiyse)
    # bu kolon "match_league_code" olarak buraya kadar geliyor - varsa dahil et
    if "match_league_code" in player_game.columns:
        pg_cols = pg_cols + ["match_league_code"]

    pg = player_game[pg_cols].copy()
    pg = pg.rename(columns={
        "game_id": "matchId", "player_id": "playerId", "team_id": "teamId",
        "match_league_code": "league_code",
    })
...
def season_normalize_func(per90_1, xt_match, csv=False):
    ...
    # cok-ligli calisildiysa oyuncunun hangi ligde oynadigini da tasi
    # ("first" - bir sezon icinde oyuncu genelde tek ligde oynar)
    if "league_code" in df.columns:
        player_meta_agg["league_code"] = ("league_code", "first")

    player_meta = df.groupby("playerId", as_index=False).agg(**player_meta_agg)
...
def tactic_builder_func(ev):
    ...
    # eng_players_data_prep.csv icinde bu kolon "match_league_code" olarak
    # geliyor (all_dataframes_pipline'da matches.add_prefix("match_") sonucu)
    league_col = "match_league_code" if "match_league_code" in ev.columns else (
        "league_code" if "league_code" in ev.columns else None)
    has_league = league_col is not None
    ...
    if has_league:
        # bir takim bir maci hep tek ligde oynadigi icin "first" yeterli
        agg_kwargs["league_code"] = ("league_code", "first")

    team_match = tmp.groupby(["matchId", "teamId"], as_index=False).agg(**agg_kwargs)
...
def tactic_season_normalize_func(team_match, csv=False):
    ...
    if "league_code" in team_match.columns:
        # bir takim bir sezonda tek ligde oynar, "first" yeterli
        team_league = team_match.groupby("teamId", as_index=False)["league_code"].first()
        season = season.merge(team_league, on="teamId", how="left")
...
def load_team_names():
    """
    Takim ID yerine takim ADI yazabilmek icin (query_runner.py'deki TAKTIK
    MOTORU / PAS AGI menuleriyle AYNI mantik) kucuk "teamss.csv" dosyasini
    okur - butun event verisini (milyonlarca satir) sadece isim-ID eslemesi
    icin okumaya gerek yok. main() calistiktan sonra "data/" klasorunde
    teamss.csv hazir olur (teams_pipline'in ciktisi).
    """
    import pandas as pd
    df = pd.read_csv("teamss.csv")
...
def resolve_team_id(query, team_names_df, valid_ids=None):
    """
    Takim ADINDAN teamId bulur - resolve_player_name / scout_engine_func'taki
    AYNI mantik: once tam eslesme (buyuk/kucuk harf onemsiz), yoksa kelime
    bazli kismi eslesme. "Takim ID kimse bilmez, isim yazmak daha mantikli"
    geri bildirimi uzerine eklendi - query_runner.py'nin interaktif
    menusunde TAKTIK MOTORU ve PAS AGI artik boyle calisiyor, full_pipeline.py
    icinden elle sorgu yaparken de ayni kolayligi kullanabilmek icin buraya
    da eklendi.
    ...
    valid_ids verilirse (ornegin sadece team_season_tactics_normalized'da
    gecen takimlar) arama sadece o ID'ler arasinda yapilir, boylece
    Wyscout'un global teams.csv'sindeki (5 ligle alakasiz, binlerce) takimlar
    aday listesine girmez.

    Donus: 0 ya da >1 eslesme -> None (mesaj zaten basildi).
           tek eslesme -> (teamId, bulunan_team_name).
    Kullanim: team_names = load_team_names()
              resolve_team_id("Chelsea", team_names, valid_ids=team_norm["teamId"].unique())
    """
...
def tactic_cluster(norm, team_id, top_n=5, csv=False):
    ...
    Kullanim (ID biliniyorsa): tactic_cluster(team_norm, team_id=1611, top_n=5, csv=True)
    Kullanim (sadece isim biliniyorsa): once resolve_team_id ile ID'yi bul,
    ornek asagida dosyanin __main__ kismindaki TAKTIK MOTORU yorumunda.
    """
    import pandas as pd
    import numpy as np
    from sklearn.metrics.pairwise import cosine_similarity
...
def resolve_player_name(query, norm):
    ...
    player_radar_func icin eklendi, cunku orada Scout Motoru'ndaki AYNI
    "tam ad bulunamiyor" sorunu yasaniyordu.
...
def player_radar_func(norm, player_names, top_n=25):
    ...
    # scout_engine_func'taki AYNI sorun buradaydi da: "Lionel Messi" gibi
    # kisa bir isim, resmi tam adla (gobek adi dahil) tam eslesmiyor. Once
    # her ismi tam player_name'e cozuyoruz, bulunamayan/belirsiz olanlari
    # (mesaji zaten basildi) listeden cikarip devam ediyoruz.
    resolved_names = []
...
def pass_network(ev, match_id, team_id):
    ...
    dosyasina kaydeder.
    Kullanim (ID biliniyorsa): pass_network(event_df, match_id=2499719, team_id=1631)
    Kullanim (sadece takim ismi biliniyorsa, match_id'yi kim bilir ki):
    once resolve_team_id ile team_id'yi bul, sonra o takimin maclarini
    event_df icinden ("matchId", "match_match_name", "match_dateutc"
    kolonlariyla) listele - ornek asagida dosyanin __main__ kismindaki
    PAS AGI yorumunda; query_runner.py'nin interaktif menusu de bunu
    zaten otomatik yapiyor.
    """
    import pandas as pd
    import numpy as np
    import matplotlib.pyplot as plt
...
def main():
    ...
    print("1/8 - players_pipline (global) çalışıyor...")
    players_pipline(PLAYERS_CSV)
    print("2/8 - teams_pipline (global) çalışıyor...")
    teams_pipline(TEAMS_CSV)
    print("3/8 - tag2names_pipline (global) çalışıyor...")
    tag2names_pipline(TAG2NAMES_CSV)
    print("4/8 - players_games_pipline (global) çalışıyor...")
    players_games_pipline(PLAYERS_GAMES_CSV)

    print(f"5-6/8 - matches_eng_pipline + events_eng_pipline, {len(LEAGUES)} lig icin ayri ayri: {LEAGUES}")
    matches_list = []
    events_list = []
    for league in LEAGUES:
        print(f"   -> {league} isleniyor...")
        m = matches_eng_pipline(league_matches_csv(league), EUTC_OFFSET, league_code=league, csv=False)
        e = events_eng_pipline(league_events_csv(league), league_code=league, csv=False)
    ...
    # her ligin tag_XXX kolon seti farkli olabilir (sadece o ligde GECEN
    # tag'ler kolon olarak olusuyor) - birlestirince eksik kalanlar NaN
    # olur, hepsini 0'a cekip int'e geri donduruyoruz
    tag_cols_all = [c for c in events_all.columns if c.startswith("tag_")]
    events_all[tag_cols_all] = events_all[tag_cols_all].fillna(0).astype(int)

    matches_all.to_csv("matches_engg.csv", index=False)
    events_all.to_csv("event_englandd.csv", index=False)
    print(f"   -> birlestirildi: matches={matches_all.shape}, events={events_all.shape}")

    print("7/8 - eventid2_name_pipline (global) çalışıyor...")
    ...
    if "league_code" in norm.columns:
        print("norm icinde lig dagilimi:")
        print(norm["league_code"].value_counts())
    if "league_code" in team_norm.columns:
        print("team_norm icinde lig dagilimi:")
        print(team_norm["league_code"].value_counts())
...
if __name__ == "__main__":
    ...
    # Oyuncu ADIYLA calisir (tam resmi ad bilinmese de - kelime bazli
    # kismi eslesme var, "Lionel Messi" gibi kisa bir isim de bulunur):
    ...
    # Bu da ADLA calisir (resolve_player_name ile ayni fuzzy eslesme):
    # "Eden Hazard" icin radar grafik + tam istatistik bar chart cizer
    # player_radar_func(player_norm, "Eden Hazard")

    # TAKTIK MOTORU  -> okudugu: team_season_tactics_normalized.csv, teamss.csv
    # Takim ID kimse bilmez, isim yazmak daha mantikli - once resolve_team_id
    # ile ADDAN ID'ye cevir, sonra tactic_cluster'i cagir:
    # team_names = load_team_names()
    # resolved = resolve_team_id("Chelsea", team_names, valid_ids=team_norm["teamId"].unique())
    # if resolved:
    #     secilen_team_id, bulunan_isim = resolved
    #     # secilen_team_id ile taktik/stil olarak en cok benzeyen 5 takimi bulur
    #     tactic_cluster(team_norm, team_id=secilen_team_id, top_n=5, csv=True)

    # PAS AGI  -> okudugu: eng_players_data_prep.csv, teamss.csv
    # match_id yerine takim ADINDAN gidip o takimin maclarini listele,
    # aralarindan secip pass_network'u cagir (query_runner.py'nin
    # interaktif menusundeki "4) PAS AGI" secenegi bunu otomatik yapar):
    # team_names = load_team_names()
    # resolved = resolve_team_id("Chelsea", team_names, valid_ids=event_df["teamId"].dropna().unique())
    # if resolved:
    #     secilen_team_id, bulunan_isim = resolved
    #     takim_ev = event_df[event_df["teamId"] == secilen_team_id]
    #     maclar = takim_ev[["matchId", "match_match_name", "match_dateutc"]].drop_duplicates("matchId")
    #     print(maclar.sort_values("match_dateutc"))  # listeden matchId'yi sec
    #     # secilen_match_id = ...
    #     # pass_network(event_df, match_id=secilen_match_id, team_id=secilen_team_id)